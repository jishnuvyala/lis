...
class Category_master extends CI_Controller
{
    ...
    public function search()
    {
        ...
        if ($this->input->post('name')) {
            ...
            $result = $this->category_model->search($name, $active);
            ...
            $result = $this->category_model->search_full($name, $active);
            foreach ($result as $row) {
                echo "<tr>";
                echo "<td>$row->name</td>";
                if (($row->active_yesno) == 1) {
    ...
    public function add()
    {
        ...
        $checkname = $this->category_model->check_name($name);
        if ($checkname) {
            $response = array(
                "check" => true
            ...
            $status = $this->category_model->add($data);
            if ($status) {
                $response = array(
                    "status" => true,
                    "check" => false
    ...
    public function update_view()
    {
        ...
        $data['category'] = $this->category_model->get_details($id);
        $this->load->view('masters/update_category', $data);
    }
    ...
    public function update()
    {
        ...
        $checkname = $this->category_model->check_name_update($name, $id);
        if ($checkname) {
            $response = array(
                "check" => true
